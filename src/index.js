...
const Quote = ({ quote }) => {
  const quoteText = quote.quote.replace(/(\.|\?)/g, "$1|").split("|");
  return (
    <div className="quote">
      <div id="author">{quote.author} said...</div>
      <div id="text">
        {quoteText.map(t => (
          <div key={t}>{t}</div>
        ))}
...
class App extends React.Component {
  ...
  componentDidMount() {
    fetchQuotes().then(quotes => {
      const randomIndex = parseInt(Math.random() * quotes.length, 10);
      this.setState({ quotes: quotes, activeQuote: randomIndex });
    });
  }

  newQuote = () => {
    let newIndex = null;
    const { activeQuote, quotes } = this.state;

    do {
      newIndex = parseInt(Math.random() * quotes.length, 10);
    } while (newIndex === null || newIndex === activeQuote);

    this.setState({ activeQuote: newIndex });
  };

  render() {
    const { activeQuote, quotes } = this.state;
    const isLoading = typeof activeQuote !== "number";
    ...
    return (
      <div id="quote-box">
        {isLoading ? (
          ...
          <>
            <Quote quote={quotes[activeQuote]} />
            <TweetQuote quote={quotes[activeQuote]} />
